...
contract('Plutus', ([alice, bob, carol, dev, agora, minter]) => {
    beforeEach(async () => {
    ...
    context('With Polis being used to set drachmas', () => {
        ...
        it('should give out POLIS only after farming time', async () => {
            ...
            await this.validators.proposeOwner(this.plutus.address, { from: dev });
            await this.plutus.claimToken(this.validators.address, { from: dev });
            // for easier test, change POLIS to 70 and tresury to 30
            await this.plutus.setPercentage('0', 70, false, {from: dev});
            await this.plutus.setPercentage('0', 30, true, {from: dev});
        ...
        it('should distribute POLIS properly for each drachma', async () => {
            ...
            this.plutus = await Plutus.new(this.polis.address, this.validators.address, web3.utils.toWei('100'), '300',{ from: dev });
            await this.polis.proposeOwner(this.plutus.address, { from: dev });
            await this.plutus.claimToken(this.polis.address, { from: dev });
            await this.validators.proposeOwner(this.plutus.address, { from: dev });
            await this.plutus.claimToken(this.validators.address, { from: dev });
        ...
        it('should distribute POLIS between treasury and drachmas', async () => {
            ...
            await this.plutus.depositToken('0', web3.utils.toWei('100'), { from: alice });
            // Agora claims rewards at block 420
            await time.advanceBlockTo('419');
            await this.plutus.claimTreasury();
            // Alice should have 10*70 pending reward
            assert.equal((await this.plutus.pendingPolis('0', alice)).toString(), web3.utils.toWei('700'));
            // Agora should have 30*20
            assert.equal((await this.polis.balanceOf(agora)).toString(), web3.utils.toWei('600'));
            ...
            // Agora (community) claims rewards at block 430. It should have 20*30 + 30*10
            await this.plutus.claimTreasury();
            assert.equal((await this.polis.balanceOf(agora)).toString(), web3.utils.toWei('900'));
            ...
            // Agora claims again. Now it should have 1*30 (+900)
            await this.plutus.claimTreasury();
            assert.equal((await this.polis.balanceOf(agora)).toString(), web3.utils.toWei('930'));
            await time.advanceBlockTo('439');
            // Governance changes the alloc of Agora to 25 at block 440 and 441 respectively
            await this.plutus.setPercentage('0', '25', true, {from:dev});
            // At block 450, treasury 1 claims rewards
            await time.advanceBlockTo('449');
            // 40*30 + 10*100*25/95 = 1463.157894736842105263
            await this.plutus.claimTreasury();
            assert.equal((await this.polis.balanceOf(agora)).toString(), web3.utils.toWei('1463.157894736842105263'));
            // Treasury at block 451
            // 40*30 + 11*100*25/95 = 1489.473684210526315789
            await this.plutus.claimTreasury();
            assert.equal((await this.polis.balanceOf(agora)).toString(), web3.utils.toWei('1489.473684210526315789'));
            // At this point, Alice can claim 1225 + 10*1/2*70 + 1*1/2*100*70/95 + 10*1/2*100*70/95 = 1980.2631578947
            assert.equal((await this.plutus.pendingPolis('0', alice)).toString(), web3.utils.toWei('1980.263157894700000000'));
            // And Bob 175 + 10*1/2*70 + 1*1/2*100*70/95 + 10*1/2*100*70/95 = 930.263157894
            assert.equal((await this.plutus.pendingPolis('0', bob)).toString(), web3.utils.toWei('930.263157894700000000'));
        ...
        it('should distribute POLIS properly for each reward token', async () => {
            ...
            // Agora should have 10*100*30/100 + 20*100*30/170 + 10*100*30/240 = 777.941176470588235294
            assert.equal((await this.polis.balanceOf(agora)).toString(), web3.utils.toWei('777.941176470588235294'));
        });
    });
});